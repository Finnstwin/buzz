...
| `BUZZ_AGENT_REQUIRE_REPLY` | `0` | `1` enables the [reply guard](#reply-guard) — remind the model to publish when a turn is about to end with nothing posted to Buzz. |


## Reply Guard

Off by default. With `BUZZ_AGENT_REQUIRE_REPLY=1`, a turn that is about to end
without any recognized attempt to post to Buzz gets a reminder that its
assistant text is invisible to humans, and is rerolled.
