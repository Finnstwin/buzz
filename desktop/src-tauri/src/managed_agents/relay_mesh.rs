...
mod tests {
    ...
    #[test]
    fn reply_guard_is_never_enabled_implicitly() {
        let mut env = BTreeMap::new();
        apply_relay_mesh_env(
            &mut env,
            Some(RELAY_MESH_PROVIDER_ID),
        ...
        assert_eq!(
            ...
            None,
            "mesh setup must not enable an explicitly opt-in guard"
        );

        let process_env = relay_mesh_process_env(&BTreeMap::new(), "Gemma-4");
        assert_eq!(
            process_env.get("BUZZ_AGENT_REQUIRE_REPLY"),
            None,
            "mesh spawn env must not inject an explicitly opt-in guard"
        );
    }
